                                            ...
                                            size_t frm_length)
{
 ...
 /* Allocate handler */
  ...
  {
    const uchar *name_pos= extra2.application_period.str + sizeof(uint16);
    period.name.length= uint2korr(extra2.application_period.str);
    period.name.str= strmake_root(&mem_root,
                                  (char*)name_pos,
                                  period.name.length);

    const uchar *constr_pos= name_pos + period.name.length + sizeof(uint16);
...
partititon_err:
  ...
  outparam->upgrade_fields_type_handler();
...
void TABLE::upgrade_fields_type_handler(void)
{
  for (uint i= 0 ; i < s->fields ; i++)
  {
    file->upgrade_type_handler(field[i]);
  }
}
