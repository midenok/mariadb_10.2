...
{
  int res;
  if ((res= key_rec_cmp(key_p, ref1 + ORDERED_REC_OFFSET,
                        ref2 + ORDERED_REC_OFFSET)))
  {
    return res;
  }
...
extern "C" int cmp_key_rowid_part_id(void *ptr, uchar *ref1, uchar *ref2)
{
  ...
  if ((res= key_rec_cmp(file->m_curr_key_info, ref1 + ORDERED_REC_OFFSET,
                        ref2 + ORDERED_REC_OFFSET)))
  {
    return res;
  }
  if ((res= file->m_file[0]->cmp_ref(ref1 + ORDERED_REC_OFFSET + file->m_rec_length,
                                     ref2 + ORDERED_REC_OFFSET + file->m_rec_length)))
  {
    return res;
  }
  return cmp_part_ids(ref1, ref2);
