    ...
    while (extra2 + 3 <= e2end)
    {
      ...
      switch (type) {
        case EXTRA2_TABLEDEF_VERSION:
          if (fields->version) // see init_from_sql_statement_string()
          {
            if (length != fields->version_len)
              DBUG_RETURN(true);
          }
          else
          {
            fields->version_len= length;
            fields->version= extra2;
          }
          break;
        case EXTRA2_ENGINE_TABLEOPTS:
          if (fields->options)
            DBUG_RETURN(true);
          fields->options= extra2;
          fields->options_len= length;
          break;
        case EXTRA2_DEFAULT_PART_ENGINE:
          fields->part_engine = extra2;
          fields->part_engine_len = length;
          break;
        case EXTRA2_GIS:
#ifdef HAVE_SPATIAL
        {
        ...
        case EXTRA2_FIELD_FLAGS:
          if (fields->field_flags)
            DBUG_RETURN(true);
          fields->field_flags= extra2;
          fields->field_flags_len= length;
          break;
        default:
