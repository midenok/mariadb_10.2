...
static
bool fk_handle_drop(THD* thd, TABLE_LIST* table, FK_create_vector& shares,
                    bool drop_db);
...
void
FK_rename_backup::rollback(ddl_log_info &log_info)
{
  if (sa.share)
    FK_ddl_backup::rollback(log_info);
  else
    fk_drop_shadow_frm(log_info, old_name);
}
