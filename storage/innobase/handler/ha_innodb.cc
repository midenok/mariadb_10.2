...
foreign_push_index_error(trx_t* trx, const char* operation,
			 const char* create_name, const char** columns,
			 fkerr_t index_error, ulint err_col,
			 dict_index_t* err_index, dict_table_t* table)
{
	switch (index_error) {
	case FK_SUCCESS:
		break;
	...
	while (Key* key = key_it++) {
		...
		if (tmp_table) {
			ib_foreign_warn(m_trx, DB_CANNOT_ADD_CONSTRAINT,
					...
					"constraint"
					" failed. Temporary tables can't have "
		...
		if (!index) {
			foreign_push_index_error(m_trx, operation, create_name,
						 column_names, index_error,
						 err_col, err_index, table);
			return (DB_CANNOT_ADD_CONSTRAINT);
		}
		...
		ut_ad(i == j); // See ER_WRONG_FK_DEF in
...
int
create_table_info_t::create_table(bool create_fk)
{
	int		error;
	int		primary_key_no;
	uint		i;
	...
	if (const char* stmt = innobase_get_stmt_unsafe(m_thd, &stmt_len)) {
		dberr_t err = DB_SUCCESS;
		if (create_fk) {
			err = create_foreign_keys();
		}
		if (err == DB_SUCCESS) {
			/* Check that also referencing constraints are ok */
			dict_names_t fk_tables;
			err = dict_load_foreigns(m_table_name, NULL, false,
